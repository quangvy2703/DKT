...
def get_data_user_sep(data_path):
    ...
    for user_path in tqdm(user_path_list, desc="Reading {} dataset.".format(data_path)):
        with open(data_path + user_path, 'rb') as f:
            ...
            user_spl = user_path.split('_')
            user_infos.append({'row_id': user_spl[2], 'group_id': user_spl[1]})
    return user_infos, sample_infos, num_of_users


def get_data_user_sep_test(data_path):
    # almost same as get_sample_info
    # for user separated format data
    sample_infos = []
    # get list of all files
